...
import { checkUserPath, get, partyDeptPath } from '../../utils'

const SchemaField = createSchemaField({
  components: { FormLayout, FormItem, TreeSelect, ArrayItems, Space, Select },
  scope: {
    async partyNameScope(field) {
      const data = await get(partyDeptPath.getLabelValue)
      if (data) {
  ...
  return <Form form={form}>
    <SchemaField>
      <SchemaField.Void x-component="FormLayout" x-component-props={{ labelCol: 6, wrapperCol: 16 }}>
        ...
        <SchemaField.Array name="partyNameList" title="党支部" x-decorator="FormItem" x-component="ArrayItems">
          <SchemaField.Void x-component="Space">
            <SchemaField.Number
              name="partyName" required x-decorator="FormItem" x-component="TreeSelect"
              x-component-props={{ style: { width: 315 }, treeDefaultExpandAll: true }}
              x-reactions="{{partyNameScope}}"
            />
            <SchemaField.Void x-decorator="FormItem" x-component="ArrayItems.Remove"/>
          </SchemaField.Void>
          <SchemaField.Void x-component="ArrayItems.Addition" title="添加党支部"/>
        </SchemaField.Array>
      </SchemaField.Void>
    </SchemaField>
  </Form>
