...
export default (props) => {
  ...
  useEffect(async () => {
    ...
    if (record) {
      ...
      //新增
      form.query('*(buttonType,buttonPosition,icon)').forEach(field => field.display = 'none')
    }
  }, [])
  ...
  return <Form form={form} layout="horizontal">
    <SchemaField>
      ...
      >
        ...
        <SchemaField.String
          name="buttonPosition" required title="按钮位置" x-decorator="FormItem" x-component="Radio.Group"
          enum={[
            { label: '工具条', value: '工具条' },
            { label: '操作', value: '操作' }
          ]}/>
        <SchemaField.String name="name" required title="权限名称" x-decorator="FormItem" x-component="Input"/>
        <SchemaField.String name="icon" title="图标" x-decorator="FormItem" x-component="Input"/>
        <SchemaField.Number name="sort" title="排序" x-decorator="FormItem" x-component="NumberPicker"/>
